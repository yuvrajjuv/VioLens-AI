...
const ExpandableCard = React.forwardRef<HTMLDivElement, ExpandableCardProps>(
  (
    ...
    useEffect(() => {
      if (isExpanded) {
        animatedHeight.set(expandedSize.height || measuredHeight);
      } else {
        animatedHeight.set(collapsedSize.height || measuredHeight);
      }
    }, [
      isExpanded,
      collapsedSize,
    ...
    return (
      ...
      >
        ...
        >
          <div className="grid grid-cols-1 rounded-lg sm:rounded-xl md:rounded-[2rem] p-1 sm:p-1.5 md:p-2 shadow">
            <div className="rounded-md sm:rounded-lg md:rounded-3xl bg-background p-2 sm:p-3 md:p-4 shadow-lg ring ring-border">
